...
@interface AddBankViewController ()<UITableViewDataSource,UITableViewDelegate,UITextFieldDelegate,UIActionSheetDelegate>{
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.row == 0) {
        ...
        [self.tef_name setPlaceholder:@"输入持卡人姓名"];
    }
    if (indexPath.row == 1) {
        ...
        self.tef_bankCode.keyboardType = UIKeyboardTypeNumberPad;
        [self.tef_bankCode setPlaceholder:@"输入银行卡卡号"];
        [self.tef_bankCode setNormalInputAccessory];
    }
    if (indexPath.row == 2) {
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.row == 2) {
        [self.tef_name resignFirstResponder];
        [self.tef_bankCode resignFirstResponder];
        [self.tef_bankZhiHang resignFirstResponder];
...
-(void)btn_sureAction{
    ...
    NSString *strUrl = [NSString stringWithFormat:@"%@%@",Development,CreateBankcard];
    NSDictionary *dic = @{@"cardNo":self.tef_bankCode.text,@"ownerName":self.tef_name.text,@"bankName":self.bankName,@"bankCode":self.bankCode,@"branchBankInfo":self.tef_bankZhiHang.text};
    self.manager = [[AFNManager alloc] init];
    [self.manager RequestJsonWithUrl:strUrl method:@"POST" parameter:dic result:^(id responseDic) {
        NSLog(@"添加银行卡:%@",responseDic);
        if ([Status isEqualToString:SUCCESS]) {
            [self.view makeToast:@"添加银行卡成功" duration:1.0 position:@"center"];
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string
{
    ...
    if (textField== self.tef_bankCode) {
        if (string.length == 0)
            return YES;
